...
export function MapPanel({ location, spots, selectedSpotId, onSpotSelect }: MapPanelProps) {
  const defaultCenter: [number, number] = [39.8283, -98.5795]; // US Center default
  
  // Determine center based on selection or user location
  let center: [number, number] = location ? [location.lat, location.lon] : defaultCenter;
  let zoom = location ? 12 : 4;
